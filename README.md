...
`serve.py` ayarları `.env`'den okur (host, port, güvenilen proxy'ler) ve **tek
worker**'ı zorunlu kılar (nedeni: [Sunucuya taşırken](#sunucuya-taşırken)).
`uvicorn app:app --port 8000` de çalışır.
...
Kod değişince backend'i yeniden başlat (Ctrl+C, sonra `python serve.py`).
`uvicorn --reload` Windows'ta değişiklikleri zaman zaman kaçırıyor ve durdurulunca
eski worker'ı portta asılı bırakabiliyor; güvenmemek daha iyi.
...
**Telefondan denemek (aynı Wi-Fi):** `npm run dev:lan` çalıştır, terminalde
yazan `http://192.168.x.x:5173` adresini telefonda aç. Vite istemcinin gerçek
adresini `X-Forwarded-For` ile iletir; IP sınırları telefon ile bilgisayarı ayrı
sayar. Tarayıcılar yalnızca güvenli bağlamda (https ya da localhost) pano ve
kaydetme penceresi API'lerini açtığı için yerel ağ adresinde "Yapıştır" butonu
çalışmaz (uyarı gösterilir, elle yapıştırılır) ve indirme Blob katmanından gider.
